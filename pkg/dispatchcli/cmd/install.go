...
func getK8sServiceNodePort(service, namespace string, https bool) (int, error) {
	...
	kubectl := exec.Command(
		...
		"-o", fmt.Sprintf("jsonpath={.spec.ports[?(@.name==%d)].nodePort}", standardPort))
...
func runInstall(out, errOut io.Writer, cmd *cobra.Command, args []string) error {
	...
	if installService("api-gateway") {
		...
		if config.APIGateway.ServiceType == "NodePort" {
			...
			httpsPort, err := getK8sServiceNodePort(service, config.Ingress.Chart.Namespace, true)
			if err != nil {
				return err
			}
			httpPort, err := getK8sServiceNodePort(service, config.Ingress.Chart.Namespace, false)
			if err != nil {
				return err
			}
